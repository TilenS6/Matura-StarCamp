...
void PhPoint::resolveCollisions(double dt, FastCont<PhLineObst> *obst) {
    ...
    for (int i = 0; i < obst->size; ++i) {
        ...
        for (int j = 0; j < collisionGroups.size; ++j) {
            if (*collisionGroups.at(j) == obst->at(i)->collisionGroup) {
                found = true;
                break;
            }
        }
        ...
        if (touchingList[i]) {
            ...
            if (collisionLineCircle(obst->at(i)->line, nowCircle, &closest)) {
                foundCollision = true;
                double lineDir = atan2(obst->at(i)->line.a.y - obst->at(i)->line.b.y, obst->at(i)->line.a.x - obst->at(i)->line.b.x);
            ...
            if (collisionLineLine(movement, obst->at(i)->line, &closest)) {
                double lineDir = atan2(obst->at(i)->line.a.y - obst->at(i)->line.b.y, obst->at(i)->line.a.x - obst->at(i)->line.b.x);
                double sinLineDir = sin(lineDir), cosLineDir = cos(lineDir);
                double As = cosLineDir * accel.y - sinLineDir * accel.x;
                if (As < 0) continue; // if it is moving from wrong direction, in same way as normal

                // cout << "new at " << i << endl;
                foundCollision = true;
                *touchingList.at(i) = true;
        ...
        if (!foundCollision && touchingList[i]) {
            *touchingList.at(i) = false;
